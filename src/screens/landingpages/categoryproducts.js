...
const ProductsScreen = ({ navigation }) => {
    ...
    async function getdata() {
        ...
        axiosInstance.post('mobile_category/category_detail/', JsonValue)
            .then((res) => {
                if (res.data.status === 201) {
                    ...
                    let duary = [];
                    duary.push(categoryId);
                    setselectedcategory([...duary]);
    ...
    const handlesubmitform = (sortdata) => {
        ...
        if (selectedbrantlits.length === 0 && selectedcategorylist.length === 0 && fynalsort === '') {
            getdata();
