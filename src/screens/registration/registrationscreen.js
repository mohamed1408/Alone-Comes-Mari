...
const RegistrationScreen = ({navigation}) => {
  React.useEffect(() => {
    async function getdata() {
      ...
      const scanned_details_str = await AsyncStorage.getItem('scanned_details');
      const scanned_details = JSON.parse(scanned_details_str);

      onChangefirstname(username);
      onChangeemail(useremail);

      onChangefirstname(scanned_details.firstname);
      onChangelastname(scanned_details.lastname);
      setbirthday(scanned_details.dob);
      onChangeaddress(scanned_details.address);
      onChangecity(scanned_details.city);
    }
    getdata();
  ...
  const handlesubmitform = async () => {
    ...
    if (validate) {
      ...
      axiosInstance
        .post('mobile_registartion/check_availability/', JsonValue)
        .then(res => {
